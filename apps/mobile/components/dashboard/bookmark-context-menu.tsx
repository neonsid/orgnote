...
import { MenuGroup, MenuItem, MenuSeparator, MenuSubItem, MenuSubTrigger } from "@/components/ui/menu-item";
...
function BookmarkContextMenuContent({
  ...
  const menuLeft = Math.max(MENU_EDGE, Math.min(anchor.x, screenWidth - MENU_WIDTH - MENU_EDGE));
  ...
  return (
    ...
    >
      <View className="overflow-hidden rounded-xl border border-border/50 bg-card shadow-lg">
        ...
        >
          <MenuGroup>
            <MenuItem icon="open-outline" label="Open link" onPress={() => void handleOpenUrl()} />

            <MenuSeparator />

            <MenuItem
              icon={b.doneReading ? "ellipse-outline" : "checkmark-circle-outline"}
            ...
            <MenuItem icon="copy-outline" label="Copy" onPress={() => void handleCopyUrl()} />
